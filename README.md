 ...
 *   **輸入驗證**: 針對傳入 Google Sheets 的文字內容已進行 CSV Injection 防護 (自動跳脫特殊字元)。
 ...
 本專案所使用的依賴服務均為**免費**或開源方案：
 *   **Google Sheets API**: 免費配額相當充裕 (每日數萬次請求)，個人使用幾乎不會產生費用。
 *   **字型與圖示**: 使用 Google Fonts 與開源 SVG，完全免費。
 *   **依賴套件**: Node.js 套件 (Express, CORS, dotENV) 皆為開源免費授權 (MIT/Apache)。
